...
public class PresignedImageDownloader {
    ...
    public MultipartFile downloadAsMultipart(DailyCultivationPhotoResponse photo) {
            ...
            return restClient.get()
                    ...
                    .exchange((request, response) -> {
                        if (response.getStatusCode().value() != HttpStatus.OK.value()) {
                            throw new ImageDownloadException(photoId, ImageDownloadException.Reason.HTTP_ERROR);
                        }

                        HttpHeaders headers = response.getHeaders();
                        validateContentEncoding(photoId, headers);

                        long declaredLength = readContentLength(photoId, headers);

                        if (declaredLength > MAX_IMAGE_SIZE_BYTES) {
                            throw new ImageDownloadException(photoId, ImageDownloadException.Reason.CONTENT_TOO_LARGE);
                        }

                        MediaType contentType = readContentType(photoId, headers);
                        ImageFormat format = resolveImageFormat(photoId, contentType);

                        byte[] content;

                        try (InputStream inputStream = response.getBody()) {
                            // Content-Length가 없거나 부정확해도 실제 읽기 크기를 제한합니다.
                            content = inputStream.readNBytes(MAX_IMAGE_SIZE_BYTES + 1);
                        }

                        if (content.length == 0) {
                            throw new ImageDownloadException(photoId, ImageDownloadException.Reason.EMPTY_CONTENT);
                        }
                        ...
                        return new ByteArrayMultipartFile(MULTIPART_NAME, "photo-%s%s"
                                .formatted(photoId, format.extension()), format.contentType(), content);
                    });
        } catch (ImageDownloadException exception) {
            throw exception;
        } catch (RestClientException exception) {
            ImageDownloadException.Reason reason = Thread.currentThread().isInterrupted()
                    ? ImageDownloadException.Reason.INTERRUPTED : ImageDownloadException.Reason.NETWORK_ERROR;

            throw new ImageDownloadException(photoId, reason);
        }
    }

    private void validateContentEncoding(Long photoId, HttpHeaders headers) {
    ...
    private long readContentLength(Long photoId, HttpHeaders headers) {
        try {
    ...
    private MediaType readContentType(Long photoId, HttpHeaders headers) {
        try {
            return headers.getContentType();
        } catch (IllegalArgumentException exception) {
            throw new ImageDownloadException(photoId, ImageDownloadException.Reason.UNSUPPORTED_CONTENT_TYPE);
        }
    }
    ...
    private URI parseAllowedOrigin(String configuredOrigin) {
        ...
        try {
            ...
            boolean supportedPath = path == null || path.isEmpty() || "/".equals(path);
            boolean validPort = port == -1 || (port >= 1 && port <= 65_535);

            if (!isSupportedScheme(uri.getScheme()) || uri.getHost() == null
                    || uri.getRawUserInfo() != null || uri.getRawQuery() != null || uri.getRawFragment() != null
                    || !supportedPath || !validPort) {
    ...
    private void validateExpiration(DailyCultivationPhotoResponse photo) {
        if (photo.expiresAt() == null
                || !photo.expiresAt().toInstant().isAfter(Instant.now())) {
            throw new ImageDownloadException(photo.photoId(), ImageDownloadException.Reason.EXPIRED_URL);
        }
    }

    private URI parseUri(Long photoId, String presignedUrl) {
        if (presignedUrl == null || presignedUrl.isBlank()) {
            throw new ImageDownloadException(photoId, ImageDownloadException.Reason.INVALID_URL);
        }
    ...
    private int effectivePort(URI uri) {
        ...
        return "https".equalsIgnoreCase(uri.getScheme()) ? DEFAULT_HTTPS_PORT : DEFAULT_HTTP_PORT;
    }

    private ImageFormat resolveImageFormat(Long photoId, MediaType contentType) {
        if (contentType == null) {
            throw new ImageDownloadException(photoId, ImageDownloadException.Reason.UNSUPPORTED_CONTENT_TYPE);
        }

        String normalizedContentType = (contentType.getType() + "/" + contentType.getSubtype())
