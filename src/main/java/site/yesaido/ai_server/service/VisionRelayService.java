...
public class VisionRelayService {
    ...
    private static final String MUSHROOM_HEALTH_CHECK_V1 =
    ...
    public GrowthRecord analyzeAndSave(
        ...
        if (existingRecord.isPresent()) {
            return returnMatchingExistingOrThrow(
                    photoId,
                    cultivationId,
                    existingRecord
            );
        ...
        MultipartFile image =
                presignedImageDownloader.downloadAsMultipart(photo);
        ...
        JsonNode analysisData =
                toAnalysisData(photoId, response);
        ...
        try {
            ...
            return returnMatchingExistingOrThrow(
                    photoId,
                    cultivationId,
                    growthRecordRepository.findByCultivationPhotoId(
                            photoId
                    )
            );
        }
    ...
    private void validatePhoto(
        ...
        Long photoId =
                photo == null ? null : photo.photoId();

        if (photo == null
                || photo.cultivationId() == null
                || photo.cultivationId() <= 0
                || photoId == null
                || photoId <= 0) {
            throw new VisionAnalysisException(
                    photoId,
                    Reason.INVALID_PHOTO_CONTRACT
            );
        }
    }

    private void validateVisionResponse(
            Long photoId,
            VisionResponse response
    ) {
        if (response == null
                || !MUSHROOM_HEALTH_CHECK_V1.equals(
                response.analysisType()
        )
                || (!SUCCESS.equals(response.status())
                && !NO_MUSHROOM_DETECTED.equals(
                response.status()
        ))
                || isNullOrBlank(response.detectorModel())
                || isNullOrBlank(response.healthModel())
                || response.thresholds() == null
        ...
        if (!isValidProbability(thresholds.detection())
                || !isValidProbability(
                thresholds.minDetectionConfidence()
        )
                || !isValidProbability(
                thresholds.healthUncertain()
        )) {
            throw invalidResponse(photoId);
        }
        ...
        if (NO_MUSHROOM_DETECTED.equals(response.status())
                && !results.isEmpty()) {
            throw invalidResponse(photoId);
        }
    ...
    private void validateResult(
        ...
        if (result == null
                || isNullOrBlank(result.species())
                || isNullOrBlank(result.speciesCode())
                || result.speciesClassId() == null
                || result.speciesClassId() < 0
                || result.detectedCount() == null
                || result.detectedCount() <= 0
                || !isValidProbability(
                result.detectionConfidence()
        )
                || !isValidProbability(
                result.detectionConfidenceMin()
        )
                || result.detectionConfidenceMin()
                > result.detectionConfidence()
                || (!HEALTHY.equals(result.healthStatus())
                && !DISEASE_SUSPECTED.equals(
                result.healthStatus()
        )
                && !UNCERTAIN.equals(
                result.healthStatus()
        ))
                || !isValidBoundingBox(result.bbox())
                || !isValidBoundingBox(result.cropBbox())) {
            throw invalidResponse(photoId);
        }
    ...
    private void validateHealthProbabilities(
            Long photoId,
            Result result
    ) {
        Double healthConfidence =
                result.healthConfidence();

        Double healthyProbability =
                result.healthyProbability();

        Double diseaseSuspectedProbability =
                result.diseaseSuspectedProbability();
        ...
        if (UNCERTAIN.equals(result.healthStatus())
                && !(allNull || allPresent)) {
            throw invalidResponse(photoId);
        }
    ...
    private JsonNode toAnalysisData(
            Long photoId,
            VisionResponse response
    ) {
        JsonNode analysisData;
    ...
    private GrowthRecord returnMatchingExistingOrThrow(
            Long photoId,
            Long cultivationId,
            Optional<GrowthRecord> existingRecord
    ) {
        return existingRecord
                .filter(record -> Objects.equals(
                        cultivationId,
                        record.getCultivationId()
                ))
                .orElseThrow(
                        () -> new VisionAnalysisException(
                                photoId,
    ...
    private VisionAnalysisException invalidResponse(
            Long photoId
    ) {
